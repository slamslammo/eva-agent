...
from .config import LoopControl, LifecycleConfig, RuntimeConfig, build_runtime_config
...
def run_runtime(config: RuntimeConfig) -> RunSummary:
    ...
    runtime = LifecycleRuntime(store, instance_guard, config.lifecycle)
...
def main() -> None:
    ...
    config = build_runtime_config(args.runtime_dir, lifecycle=lifecycle, control=control)
