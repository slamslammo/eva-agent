...
| Candidate generation | Implemented at a stronger baseline. Candidate generation now materializes candidates from anchor-admitted `ActionDomain` schemas instead of assuming the full fixed set first. | Should produce a broader candidate space shaped by drive, anchor, memory, and learned skill. | The admitted schema set is still narrow and compatibility-oriented. | Tighten | Broaden the action-domain schema vocabulary and upstream admission sources before widening the external action surface. |
| Anchor restriction | Implemented at a stronger baseline. Anchors now compute a pre-generative `ActionDomain`, can narrow admitted schemas using heartbeat timing, and still keep `apply_structural_anchors(...)` as a bounded compatibility mirror for parameter-domain injection. | Should become a fuller pre-generative anchor system. | Current anchor sources are still limited, and pre-generative restriction only covers a narrow runtime/heartbeat slice so far. | Tighten | Expand anchor sources and semantics through `ActionDomain` admission rather than falling back to post-hoc filtering. |
| Value judgment | Implemented at a stronger baseline. Candidate scoring now reads continuous `drive_levels` through explicit candidate impact schemas, while signal pressure, anchor/runtime disposition, and bounded learning remain secondary shaping inputs. | Should become a richer but still interpretable judgment layer inside a fuller reasoning core. | The current candidate set is still narrow and compatibility-oriented, and conflict semantics remain comparatively simple. | Tighten | Broaden candidate semantics and conflict handling on top of the landed drive-weighted baseline rather than falling back to signal-led scoring. |
| Peer-circuit / independent selection | Implemented at a minimal baseline. Release selection and outcome closeout now have a dedicated `peer_circuit/` subtree. | Candidate selection should become an independent peer-circuit layer, distinct from reasoning and from execution authorization. | The current peer-circuit remains simple and still operates over a narrow compatibility-scoped candidate space. | Stable baseline | Deepen peer-circuit semantics before widening the external action surface. |
| Mediator / default inhibition | Implemented at a stronger baseline. Release is still withheld by default, but `ReleaseToken` is now minted only by the mediator, validated again at tool-edge execution time, threaded through lifecycle handoff, and kept runtime-only rather than serialized into persisted audit or history artifacts. | Must remain an independent release authority with default inhibition. | The release vocabulary and action surface are still narrow, even though execution authority is now structurally gated. | Stable baseline | Preserve mediator-owned runtime authority while expanding the candidate and release vocabulary around the same mediated gate. |
...
4. broaden candidate, retrieval, and tool-edge semantics without weakening kernel, anchor, or mediator authority.
