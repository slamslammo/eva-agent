...
| Anchor restriction | Implemented at a minimal baseline. Current anchors mostly inject runtime-validity and life-boundary gate conditions into candidate domains. | Should become a fuller pre-generative anchor system. | Current anchors are still limited to a narrow runtime-gate subset of the full target anchor space. | Tighten | Expand anchor sources and semantics without turning anchors into post-hoc filtering. |
| Value judgment | Implemented at baseline. Drive, signal pressure, and bounded learning already affect scoring and disposition. | Should become a richer but still interpretable judgment layer inside a fuller reasoning core. | Scoring semantics are still narrow, and the broader reasoning structure remains minimal. | Tighten | Improve explicit weighting and conflict handling rather than hiding behavior in opaque heuristics. |
...
| Full cognitive retrieval | Not yet implemented at target depth. Baseline retrieval helpers already exist for recent outcomes, habit bias, response history, and cognitive-memory stubs, but not yet as a richer salience-weighted cognitive retrieval layer. | Should provide salience-weighted episodic retrieval and richer cognitive memory use. | Richer episodic retrieval, stronger salience composition, and broader cognitive-memory use are still missing. | Missing piece | Deepen the landed retrieval owner before attempting higher-order self-model or social-layer expansion. |
