...
| Peer-circuit / independent selection | Not yet implemented as an independent layer. Candidate selection still happens inside a simplified mediator flow over narrow compatibility candidates. | Candidate selection should become an independent peer-circuit layer, distinct from reasoning and from execution authorization. | Selection is still folded into mediator decision flow. | Missing piece | Separate candidate-selection semantics before broadening the external action surface. |
...
| Tool edge / action surface | Still minimal. Practical execution is still routed through the narrow compatibility response bridge rather than a broader mediated tool edge. | Should become a wider but still mediator-owned external action surface. | The current action surface is still response-bridge scoped rather than tool-registry scoped. | Missing piece | Grow a mediated action surface inside L3 instead of enlarging the compatibility bridge. |
