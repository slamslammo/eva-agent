...
| Existence semantics declaration (eight-field dataclass) (blueprint §2.7 / §12.7 / §3.8, rev2) | 1.1 `Scenario-declared existence semantics (eight-field dataclass...)` | landed | The declaration / config boundary is written into §12.7; the first seven are human-readable declarations, and item 8 `clock_source` is a dataclass field that has landed, and kernel consumption of it has now landed as well (see the next row). |
...
| Kernel consumption of `clock_source` to select cadence rhythm source (blueprint §2.7 Clock row / §12.7 item 8 / §2.7 derived constraint #3 / §15.4 anti-pattern #3 contract target) | 1.1 `Kernel consumption of declared clock_source to select cadence rhythm source` | landed | The kernel reads `clock_source` from the active existence semantics at runtime construction and branches its cadence accounting on it: `step` honors the bridge's deferred signal (scenario time advances only when `env.step` is invoked), while `wall_clock` enforces the attempt==scenario_step invariant; scenario-time freeze never freezes heartbeat / lease / liveness. The "declaring `clock_source` is the correct way" framing in §2.7 / §15.4 anti-pattern #3 is now a landed status. |
...
| L3 reasoning core forms candidates (dlPFC producer) | 1.4 `L3 reasoning core — dlPFC candidate producer` | landed (phase 2 (a)) | Round 1.G (blueprint §7.4): round-1e reorder-proposer + ≤0.12 advisory superseded (drift, round-1f); replaced by a behavior-preserving `CandidateProducer` seam (`HeuristicCandidateProducer`). Phase 2 (a) landed (G2-approved): the live `LLMCandidateProducer` annotates each candidate with an `action_hint` — the LLM's causal lever, the concrete action within the drive-locked posture — threaded through `release_context` and consumed by the Crafter bridge (priority over default/prior). Causal path proven on live DeepSeek (LLM hint=X → executed=X); model-off byte-equivalent. |
