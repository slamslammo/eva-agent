...
| Kernel consumption of declared `clock_source` to select cadence rhythm source (blueprint §2.7 / §12.7 item 8) | `eva/kernel/lifecycle.py::LifecycleRuntime._update_scenario_counters`, `eva/kernel/main.py` | production | The kernel reads `clock_source` from the active existence semantics at runtime construction and branches its cadence accounting on it: `step` honors the bridge's deferred signal (scenario time advances only when `env.step` is invoked), while `wall_clock` enforces the attempt==scenario_step invariant regardless of any bridge defer flag. Scenario-time freeze never freezes heartbeat / lease / liveness (only `scenario_step_index` is gated). Currently only Crafter declares step; all other scenarios use the wall_clock default | — |
