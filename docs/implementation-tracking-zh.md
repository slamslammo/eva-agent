...
| L3 reasoning core — dlPFC 候选产出器（理论 v0.5 §8.6.3 / 蓝图 §7.4） | `eva/l3_deliberation/reasoning/candidate_producer.py`、`eva/l3_deliberation/runtime.py` | partial | Round 1.G phase 1：round-1e reorder-proposer **与** ≤0.12 advisory **退役（drift）**——round-1f 证 proposer 因果惰性（mediator 选择顺序无关 max-by-score）、≤0.12 有害（把选择推向 passivity）且无理论依据。改为 `CandidateProducer` seam：`HeuristicCandidateProducer`（确定性、= `build_candidates`、behavior-preserving 默认）在 anchor 框定域内产候选；peer-circuit 选择 + mediator 释放权不变。 | Phase 2（live-LLM 产出器 + drive 底线防 passivity）待 A+用户敲定因果设计 |
