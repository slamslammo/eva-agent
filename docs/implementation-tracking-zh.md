...
| L3 reasoning 提议路径（anchor 受限的 proposer 塑形 considered candidate set） | `eva/l3_deliberation/reasoning/proposer.py`、`eva/l3_deliberation/runtime.py`、`eva/l3_deliberation/contracts.py` | partial | Round 1.E 填补 reasoning "虚位"：`Proposer`（HeuristicProposer 默认；ModelBackedProposer 复用上游 `advisory_context`、不新增 LLM 调用）在 admission 与 assessment 之间**仅在 anchor admitted 域内**对候选 rank/塑形；域外 proposal 拒绝 + 记录；audit 记 `proposal_id → selected candidate` 归因。peer-circuit 选择 + mediator 释放权不变；`local_rule_based` inert / behavior-preserving。 | LLM-reasoning 长跑评估（存活 / achievements / 归因占比）为后续单独 round（Round 1.E §8） |
