...
- **H-2c TODO（进冻结 owner body，按 A 的 (i) 条件）**：剩 3 项需 patrol-time owner-internal 值 → 先建 **current-trace 机制**（EVA 串行单 agent → 模块级 current sink+turn_index，lifecycle 在 turn 起点 set/reset），再：**`l2.approach_delta`（flag-gated 只读 owner-hook @ `drive_state._approach_target_delta`，A 批准例外，须专门字节等价测试）** + `l1.rate_sense`（build_rate_context seam 或 hook）+ `l1.threshold_classify`（seam 优先；若 rationale 仅体内可见=第 3 处 owner-hook→先 flag A）。**delicate、单独 tick 做。**
