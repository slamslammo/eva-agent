...
**当前状态**：**G2_REQUESTED**（round-1h 全 slice 完成）。H-1 + H-2(a–d) + H-3(a/b) + H-4 DONE：P1a+P1b 全部 13 transform/snapshot emit + 整链 conformance 自证；含 A 批准的 2 处 owner-hook（`l2.approach_delta`/`candidate_scoring`，专门字节等价测试）；**528 绿**、flag-off 字节等价、opt-in 0-token。**待 A G2 gate。**
...
- **H-2a ✅ DONE**：P1a 批1 中**纯 seam 组装的 4 项**（`l1.signal_publish` / `l2.broadcast` / `anchor.admit` + `drive_state` snapshot）。lifecycle 慢车道 `run_deliberation` 后 `_emit_p1a_seam_trace`（gated by `trace_sink.enabled`）从 `deliberation_input`(gate-state/signal_batch/drive_broadcast) + `deliberation_audit`(candidates) **已返回数据**组装——**未触任何 owner 函数体**。typed parents（pressure / top_drive_bias）。`_trace_turn_index` 仅 tracing 时自增 → flag-off 字节等价。**conformance 集成测试**（`EVA_TRACE=1` stub、0 token）验 4 事件出现 + turn_index 对齐 + 信封 + run_meta。**全量回归 525 绿**。
- **H-2b ✅ DONE**：`l1.raw_observation` seam emit（无 owner body）。lifecycle `_emit_p1a_seam_trace` 扩展，从 `extra_shared_facts.agent_observation`（patrol 已取）发 agent 实际消费的**符号 9×7 grid**（local_view cells+center + facing + life/inventory panel + step）→ `raw_observations/turn-N.json` + cognitive_trace 内联索引、共享 turn_index。RGB（像素）非符号 obs 内容、留 wrapper 侧 refinement。conformance 测试加断言（`l1.raw_observation` 出现 + `raw_observations/` 有文件）。**525 绿**、flag-off 字节等价、未触 owner body。
- **H-2c ✅ DONE（A 批准的 owner-hook #1）**：current-trace 机制 + `l2.approach_delta` owner-hook。① trace_sink 加**模块级 current-trace**（EVA 串行→`set_current_trace`/`reset_current_trace`/`current_trace_sink`/`current_trace_turn_index`，未设=NullTraceSink no-op）。② lifecycle：patrol 起点 `set_current_trace(sink, turn_index)`（仅 tracing）→ patrol-time owner code 可读；turn_index 改为每 patrol turn 起点分配、owner-hook + seam emit 共享、turn 末 `+1`+`reset`（仅 tracing→flag-off 字节等价）。③ `drive_state._approach_target_delta`：末尾 `result=...; if current_trace_sink().enabled: emit l2.approach_delta(target/delta/approach_rate/contributors); return result`——**只读、不改 return/控制流**（A 条件 b）。**专门字节等价单测**（A 条件 c）：flag-off vs flag-on `update_drive_state` 返回的 DriveState 行**逐字段相同** + flag-on 发 l2.approach_delta 带 turn_index；conformance 集成测试加断言 l2.approach_delta 出现。**全量回归 526 绿**、flag-off 字节等价、**仅触 A 批准的 drive_state owner body**。
- **H-2d ✅ DONE（P1a 全齐！）**：`l1.threshold_classify` + `l1.rate_sense` **纯 seam 组装**——二者的值（dimension status / evidence.reason / rate_context）都在 `patrol_result.snapshot.dimensions[]` 上、seam 可见，**无需第 3 处 owner-hook**（不用 flag A）。lifecycle `_emit_p1a_seam_trace` 加 `patrol_snapshot` 参 + 两 emit（threshold_classify 出 per-dim status+reason；rate_sense 出 per-dim direction，仅当有 rate_context）；修正 L1 链 parents（raw_observation→threshold_classify→rate_sense/signal_publish）。conformance 加断言 threshold_classify 出现。**526 绿**、flag-off 字节等价、未触 owner body。**→ P1a 批1 完整**（raw_observation/threshold_classify/rate_sense/signal_publish + approach_delta/broadcast + drive_state snapshot + anchor.admit + run_meta + raw_observations/）。
- **H-3a ✅ DONE（P1b L3 链 seam）**：5 个 L3 transform **纯 seam 组装**——`l3.candidate_produce`/`l3.assess_score`/`l3.decide_release`/`mediator.release` 从 `deliberation_audit`（candidates/assessments/release_decision）在 `_emit_p1a_seam_trace` emit；`bridge.resolve_action` 从慢/快车道汇合点的 `response_summary`（selected_action + reason，捕 `crafter_llm_action_hint_selection` 因果点）emit。**OFC(value_judgment) / mediator 只读、未改**。按 A 裁示 `l3.decide_release`+`mediator.release` 都从 `decide_release` 返回值 emit、**不双 hook**；二者字段**不冗余**（decide_release=选择 disposition: selected_candidate_id/action/outcome；mediator.release=释放门: outcome/release_authorized/action_hint）→ 两条事件成立、**无需 flag A 合并**。conformance 加断言全 5 个 + bridge.resolve_action 出现。**526 绿**、flag-off 字节等价、未触 owner body。
- **H-3b ✅ DONE（A 批准的 owner-hook #2）→ P1b 全齐 + 全部 emit 完成**：snapshot `candidate_scoring` @ `value_judgment.assess_candidates`。分解值（drive_weighted=drive_score / projection=projection_score / learning_bias / habit=habit_priority_bonus / advisory=0 退役 / final_score=score）仅函数体内可见 → flag-gated 只读 emit（用 H-2c current-trace 机制）。loop 顶零初始化 3 个分量变量（additive、allow 分支照常重赋、assessment 输出不变）；emit 在 `assessments.append` 后、gated。**专门字节等价单测**（flag-off vs flag-on `assess_candidates` 返回 `CandidateAssessment` 列表 `==` 相同 + flag-on 每候选发 1 candidate_scoring snapshot 带分解）；conformance 加断言。**527 绿**、flag-off 字节等价、**仅触 A 批准的 value_judgment owner body**（owner-hook #2/2，无第 3 处）。
- **H-4 ✅ DONE → 置 G2_REQUESTED**：整链 conformance 自证测试（stub、0 token）。`test_full_chain_schema_conformance_and_replay`：每事件满足 §2 信封（9 键全 + continuity_state ∈ {alive/terminated/new_individual/inherited}）+ §3 transform（layer ∈ 7 类、transform_id/code_anchor 非空、parents=[{id,edge_type∈词汇}]）/ §4 snapshot（snapshot_type+values）；**turn_index 单调非降**（可对齐 replay）；**链连通性**（每 parent.id ∈ 实际 emit 的 transform_id 集）；run_meta §1 全字段填实。**全量回归 528 绿**。**→ round-1h 所有 slice 完成，置 `G2_REQUESTED` 交 A gate。**
