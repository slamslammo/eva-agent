...
- **H-3 TODO（P1b 批2，L3 链）**：`l3.candidate_produce`（producer 返回值 seam）/ `l3.assess_score`（assess 返回值 seam，OFC 冻结只读）/ `l3.decide_release`+`mediator.release`（`decide_release` 返回值 seam，按 A：同函数→都从返回值 emit、不双 hook，字段冗余则 flag A 合并）/ `bridge.resolve_action`（ResponseSelection，捕 `crafter_llm_action_hint_selection` 因果点）/ snapshot `candidate_scoring`（**owner-hook #2 @ value_judgment.assess_candidates**，分解值仅体内可见、字节等价证）。
