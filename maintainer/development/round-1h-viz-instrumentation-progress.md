...
**当前状态**：**G1 APPROVED → 执行中**。**H-1 + H-2(a/b/c/d) DONE → P1a 批1 完整**（全 8 transform + run_meta + raw_observations，含 A 批准的 `l2.approach_delta` owner-hook；526 绿、flag-off 字节等价）。下一步 **H-3**（P1b 批2 = L3 链 + `candidate_scoring` owner-hook #2）。
...
- **H-3b TODO（owner-hook #2）**：snapshot `candidate_scoring` @ `value_judgment.assess_candidates`——分解值（drive_weighted/projection/learning_bias/final）仅函数体内可见 → flag-gated 只读 owner-hook（A 批准例外 #2），用 current-trace 机制（H-2c 已建）+ 专门字节等价测试。delicate、单独 tick。
- **H-4 TODO**：conformance 短 trace（stub、0 token）整链 replay + schema conformance + identity 自证 → `G2_REQUESTED`。
