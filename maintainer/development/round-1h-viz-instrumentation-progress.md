...
**当前状态**：**G1_REQUESTED**（待 A 批 intake，尤其 2 处 owner-hook 判断 + 1 处 anchor 更正）。
...
## 5. slice 计划
- **H-1**：`TraceSink` + `run_meta` + 通用信封 + `EVA_TRACE` flag 接线（lifecycle 构造 + threading；no-op when off）。
- **H-2**：P1a 批1（`l1.*` + `l2.*` + `anchor.admit` + `drive_state` snapshot + `raw_observations/`）→ 自证 schema 一致 + flag-off 字节等价 + 516 绿。
- **H-3**：P1b 批2（`l3.*` + `mediator.release` + `bridge.resolve_action` 因果点 + `candidate_scoring` snapshot）→ 同样自证。
- **H-4**：conformance 短 trace（stub、0 token）+ 字节等价自证 → 置 `G2_REQUESTED`。
