...
- **H-2d TODO（P1a 收尾）**：`l1.rate_sense`（`build_rate_context` seam 优先）+ `l1.threshold_classify`（avatar_state，seam 优先；若 rationale 仅体内可见=第 3 处 owner-hook→**先 flag A**）。完成即 P1a 全齐 → H-3 P1b。
