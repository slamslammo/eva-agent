...
**当前状态**：**G1 APPROVED → 执行中**。**H-1 + H-2(a–d) + H-3(a/b) DONE → P1a + P1b 全部 emit 完成**（13 transform/snapshot 全落地，含 A 批准的 2 处 owner-hook：`l2.approach_delta` + `candidate_scoring`；**527 绿**、flag-off 字节等价、专门 owner-hook 字节等价测试）。下一步 **H-4**（conformance 整链 replay + schema 自证 → `G2_REQUESTED`）。
...
- **H-4 TODO**：conformance 短 trace（stub、0 token）整链 replay + schema conformance + identity（done→terminated）自证 → 置 `G2_REQUESTED`。
