...
- **H-2b TODO**：P1a 批1 剩 4 项 —— `l1.raw_observation`（wrapper，9×7 grid+RGB→`raw_observations/`）、`l1.threshold_classify`（avatar_state，seam 优先）、`l1.rate_sense`（build_rate_context）、**`l2.approach_delta`（flag-gated 只读 owner-hook @ `drive_state._approach_target_delta`，A 批准的 2 例外之一，须证字节等价）**。
