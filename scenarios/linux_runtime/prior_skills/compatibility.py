"""Linux runtime prior-skill derivation policy for Phase A."""
...
def habit_skill_match_for_candidate_profile(candidate_profile: str | None) -> bool:
    """Return whether this candidate profile participates in the current Linux prior-skill track."""
...
def derive_habit_skills(
    *,
    ...
    """Derive Linux runtime habit-skill summaries from current learning artifacts."""
        ...
        skills.append(
            {
                ...
                "preferred_action": (
                    str(entry.get("preferred_action"))
                    if entry.get("preferred_action") is not None
                    else None
                ),
...
def summarize_habit_bias(learning_outcomes: list[dict[str, Any]], *, situation_key: str) -> list[dict[str, Any]]:
    """Summarize recurring Linux runtime outcomes into evidence-weighted habit-bias entries."""
...
def situation_key_from_learning_outcome(record: dict[str, Any]) -> str:
    """Return the normalized situation key recorded with one Linux runtime learning outcome."""
